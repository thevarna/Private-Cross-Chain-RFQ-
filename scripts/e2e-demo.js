...
const PROGRAM_ID = new web3_js_1.PublicKey(process.env.PROGRAM_ID || "PRVrFQd3eBKaxK3TEvdA2FPLQiSfGjH7jYHMEsGhsXM");
const USDC_MINT = new web3_js_1.PublicKey("4zMMC9srt5Ri5X14GAgXhaHii3GnPAEERYPJgZJDncDU");
const IKA_PID = new web3_js_1.PublicKey("87W54kGYFQ1rgWqMeu4XTPHWXWmXSQCcjm8vCTfiq1oY");
const ENCRYPT_PID = new web3_js_1.PublicKey("4ebfzWdKnrnGseuQpezXdG8yCdHqwQ1SSBHD3bWArND8");
...
const logPurple = (text) => console.log(`\x1b[35m${text}\x1b[0m`);
...
async function runDemo() {
    ...
    const dwalletPubkey = web3_js_1.Keypair.generate().publicKey; // Mock dWallet pre-generated via Ika gRPC
    const [rfqPda, rfqBump] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("rfq"), maker.publicKey.toBuffer(), salt], PROGRAM_ID);
    const [vaultPda, vaultBump] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("vault"), rfqPda.toBuffer()], PROGRAM_ID);
    const [bidPda, bidBump] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("bid"), rfqPda.toBuffer(), taker.publicKey.toBuffer()], PROGRAM_ID);
    const [encryptCpiAuthority, encBmp] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("__encrypt_cpi_authority")], PROGRAM_ID);
    const [ikaCpiAuthority, ikaBmp] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("__ika_cpi_authority")], PROGRAM_ID);
    // Mock Encrypt Ciphertexts
    const rfqPriceCt = web3_js_1.Keypair.generate().publicKey;
    const rfqSizeCt = web3_js_1.Keypair.generate().publicKey;
    const bidPriceCt = web3_js_1.Keypair.generate().publicKey;
    const bidSizeCt = web3_js_1.Keypair.generate().publicKey;
    const matchResultCt = web3_js_1.Keypair.generate().publicKey;
    // Mock System Params
    const encryptConfig = web3_js_1.Keypair.generate().publicKey;
    const encryptDeposit = web3_js_1.Keypair.generate().publicKey;
    const networkEncKey = web3_js_1.Keypair.generate().publicKey;
    const eventAuthority = web3_js_1.Keypair.generate().publicKey;
    const decryptionRequest = web3_js_1.Keypair.generate().publicKey;
    const messageApproval = web3_js_1.Keypair.generate().publicKey;
